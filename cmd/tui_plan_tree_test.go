...
// TestBuildBeadTree_WithDatabaseClient tests using the database client path
func TestBuildBeadTree_WithDatabaseClient(t *testing.T) {
	// This test would require a mock database client
	// For now, we verify the function doesn't crash with nil client
	items := []beadItem{
...
// TestBuildBeadTree_SearchMode tests that search mode doesn't affect parent fetching
func TestBuildBeadTree_SearchMode(t *testing.T) {
	items := []beadItem{
		{id: "parent", title: "Parent", status: "closed", priority: 1, isClosedParent: true},
		{id: "child", title: "Child", status: "open", priority: 2, dependencyCount: 1},
	}
	...
	if len(result) != 2 {
		t.Errorf("expected parent to be fetched even in search mode, got %d items", len(result))
	}
}
